...
CDC.init = function(fileList_,params) {
	...
	CDC.flag_overwrite = params.flag_overwrite;
	CDC.flag_res_to_folders = params.flag_res_to_folders;
	CDC.queueCurrent = 0;
	...
	if(CDC.batch_outputDir == null) CDC.outputDir_Info = ". (Same as source)"; else {
		CDC.batch_outputDir = js_node_Path.normalize(CDC.batch_outputDir);
		CDC.outputDir_Info = CDC.batch_outputDir + "\\";
	}
	djNode_tools_LOG.log("-- CDC RUN PARAMETERS --",null,{ fileName : "CDC.hx", lineNumber : 222, className : "CDC", methodName : "init"});
	djNode_tools_LOG.log(" mode      = " + CDC.batch_mode,null,{ fileName : "CDC.hx", lineNumber : 223, className : "CDC", methodName : "init"});
	djNode_tools_LOG.log(" quality   = " + CDC.batch_quality,null,{ fileName : "CDC.hx", lineNumber : 224, className : "CDC", methodName : "init"});
	djNode_tools_LOG.log(" outputDir = " + CDC.batch_outputDir,null,{ fileName : "CDC.hx", lineNumber : 225, className : "CDC", methodName : "init"});
	djNode_tools_LOG.log(" tempdir   = " + CDC.batch_tempdir,null,{ fileName : "CDC.hx", lineNumber : 226, className : "CDC", methodName : "init"});
	djNode_tools_LOG.log(" files     = " + Std.string(CDC.fileList),null,{ fileName : "CDC.hx", lineNumber : 227, className : "CDC", methodName : "init"});
	djNode_tools_LOG.log("-------------------------",null,{ fileName : "CDC.hx", lineNumber : 228, className : "CDC", methodName : "init"});
};
CDC.processNextFile = function() {
	...
	inf.outputDir = CDC.batch_outputDir;
...
CDC.createTempDir = function(par) {
	if(par.tempDir == null) par.tempDir = par.outputDir;
	par.tempDir = js_node_Path.join(par.tempDir,CDC.generateTempFolderName(js_node_Path.parse(par.input).name));
	try {
		djNode_tools_LOG.log("Creating temp directory \"" + par.tempDir + "\"",null,{ fileName : "CDC.hx", lineNumber : 294, className : "CDC", methodName : "createTempDir"});
...
CDC.isWritable = function(dir) {
	try {
		var testFile = "_cdcrush_test_file_temp";
		if(djNode_tools_FileTool.pathExists(js_node_Path.join(dir,testFile)) == false) js_node_Fs.writeFileSync(js_node_Path.join(dir,testFile),"ok");
		js_node_Fs.unlinkSync(js_node_Path.join(dir,testFile));
	} catch( e ) {
		if (e instanceof js__$Boot_HaxeError) e = e.val;
		if(!djNode_tools_FileTool.pathExists(dir)) throw new js__$Boot_HaxeError("Folder \"" + dir + "\" does not exist, or can't create"); else throw new js__$Boot_HaxeError("Can't write to output dir \"" + dir + "\" do you have write access?");
	}
};
...
Job_$Crush.prototype = $extend(djNode_task_Job.prototype,{
	start: function() {
		...
		this.add(new djNode_task_Qtask("-loadingImageInfo",function(t) {
			if(_g.par.outputDir == null) _g.par.outputDir = _g.par.inputDir;
			CDC.isWritable(_g.par.outputDir);
			try {
			...
			djNode_tools_LOG.log("Loaded " + _g.par.input,null,{ fileName : "Job_Crush.hx", lineNumber : 66, className : "Job_Crush", methodName : "start"});
			djNode_tools_LOG.log("Image TITLE = " + _g.par.cd.TITLE,null,{ fileName : "Job_Crush.hx", lineNumber : 67, className : "Job_Crush", methodName : "start"});
			var arcFile = _g.par.cd.TITLE + "." + "arc";
			_g.par.crushedArc = js_node_Path.join(_g.par.outputDir,arcFile);
			djNode_tools_LOG.log("Setting output file to " + _g.par.crushedArc,null,{ fileName : "Job_Crush.hx", lineNumber : 72, className : "Job_Crush", methodName : "start"});
			if(djNode_tools_FileTool.pathExists(_g.par.crushedArc) && !CDC.flag_overwrite) throw new js__$Boot_HaxeError("\"" + arcFile + "\" already exists. \n Run with -w to overwrite files");
			if(!CDC.createTempDir(_g.par)) {
				t._fail("Could not create tempdir at \"" + _g.par.tempDir + "\"","IO");
				return;
			}
			if(_g.par.cd.isMultiImage == false) _g.addNext(new Task_$CutTracks());
			t._complete();
		}));
		...
		this.add(new djNode_task_Qtask("Compressing",function(t3) {
			...
			arc.compress(listOfFilesToCompress,_g.par.crushedArc);
		}));
		this.add(new djNode_task_Qtask("-cleaning",function(t4) {
			...
			while(_g3 < listOfFilesToCompress.length) {
				...
				++_g3;
				djNode_tools_LOG.log("Deleting \"" + i1 + "\"",null,{ fileName : "Job_Crush.hx", lineNumber : 142, className : "Job_Crush", methodName : "start"});
				js_node_Fs.unlinkSync(i1);
			}
			djNode_tools_LOG.log("Deleting \"" + _g.par.tempDir + "\"",null,{ fileName : "Job_Crush.hx", lineNumber : 145, className : "Job_Crush", methodName : "start"});
			js_node_Fs.rmdirSync(_g.par.tempDir);
			_g.par.sizeAfter = Std["int"](js_node_Fs.statSync(_g.par.crushedArc).size);
			t4._complete();
		}));
		djNode_task_Job.prototype.start.call(this);
	}
	,addQueue_simulate: function() {
		...
		this.par.crushedArc = gamedir + gamename + ".arc";
...
Job_$Restore.prototype = $extend(djNode_task_Job.prototype,{
	processOutputFolder: function() {
		if(this.par.outputDir == null) this.par.outputDir = this.par.inputDir;
		if(CDC.flag_res_to_folders) {
			this.par.outputDir = js_node_Path.join(this.par.outputDir,js_node_Path.parse(this.par.input).name + Job_$Restore.DIR_POSTFIX);
			if(djNode_tools_FileTool.pathExists(this.par.outputDir) && !CDC.flag_overwrite) throw new js__$Boot_HaxeError("Output Dir " + this.par.outputDir + " already exists\n Use -w to force overwrite, or delete manually to avoid errors.");
			try {
				djNode_tools_FileTool.createRecursiveDir(this.par.outputDir);
			} catch( e ) {
				if (e instanceof js__$Boot_HaxeError) e = e.val;
				throw new js__$Boot_HaxeError("Cannot create " + this.par.outputDir + ".\n Do you have write access?");
			}
		} else CDC.isWritable(this.par.outputDir);
		djNode_tools_LOG.log(" - Setting Output dir to " + this.par.outputDir,null,{ fileName : "Job_Restore.hx", lineNumber : 67, className : "Job_Restore", methodName : "processOutputFolder"});
	}
	,start: function() {
		var _g = this;
		this.par = this.sharedData;
		...
		this.add(new djNode_task_Qtask("-loadcdinfo",function(t1) {
			...
			_g.par.imagePath = js_node_Path.join(_g.par.outputDir,_g.par.cd.TITLE + ".bin");
			_g.par.cuePath = js_node_Path.join(_g.par.outputDir,_g.par.cd.TITLE + ".cue");
			var c = _g.par.cd.tracks_total;
		...
		this.add(new djNode_task_Qtask("-finalize",function(t3) {
			...
			djNode_tools_LOG.log("Creating CUE at " + _g.par.cuePath,null,{ fileName : "Job_Restore.hx", lineNumber : 172, className : "Job_Restore", methodName : "start"});
			_g.par.cd.saveAs_Cue(_g.par.cuePath,"GENERATED BY CDCRUSH " + "1.1");
			djNode_tools_LOG.log("Clearing temp dir",null,{ fileName : "Job_Restore.hx", lineNumber : 175, className : "Job_Restore", methodName : "start"});
...
djNode_BaseApp.prototype = {
	...
	,init: function() {
		...
		this.addParam("-help","Display Usage info","This screen",false,false,true);
	...
	,addParam: function(command,name,description,requireValue,isDefault,hidden) {
	...
	,getParameters: function() {
		djNode_tools_LOG.log("Getting Parameters",1,{ fileName : "BaseApp.hx", lineNumber : 285, className : "djNode.BaseApp", methodName : "getParameters"});
		var cc = 2;
	...
	,onExit: function() {
		if(this.flag_force_exit) djNode_tools_LOG.log("App Quit - User Quit",null,{ fileName : "BaseApp.hx", lineNumber : 595, className : "djNode.BaseApp", methodName : "onExit"}); else djNode_tools_LOG.log("App Quit -  Normally",null,{ fileName : "BaseApp.hx", lineNumber : 597, className : "djNode.BaseApp", methodName : "onExit"});
...
Main.prototype = $extend(djNode_BaseApp.prototype,{
	init: function() {
		...
		this.addParam("-w","Force Overwrite","Overwrite any files",false,false,true);
		this.addParam("-f","Restore to Folders","Restore ARC files to separate folders",false,false);
	...
	,create: function() {
		...
		CDC.init(this.params_Input,{ mode : this.action, temp : this.getOptionParameter("-t"), sim : this.params_Options.exists("-sim"), quality : this.getOptionParameter("-q"), output : this.params_Output, flag_overwrite : this.params_Options.exists("-w"), flag_res_to_folders : this.params_Options.exists("-f")});
		CDC.processNextFile();
	}
	,processJobStatus: function(status,job) {
		...
		case "complete":
			...
			if(CDC.batch_mode == "restore") {
				...
				this.info.deletePrevLine();
				this.info.printPair("Created",inf.crushedArc);
				this.info.printPair("Number of tracks",inf.cd.tracks_total);
				this.info.printPair("Raw size",s0);
...
Task_$CutTracks.prototype = $extend(djNode_task_Task.prototype,{
	run: function() {
		...
		if(this.par.cd.isMultiImage) {
			djNode_tools_LOG.log(" - Skipping CUT, as the image is already cut",null,{ fileName : "Task_CutTracks.hx", lineNumber : 44, className : "Task_CutTracks", methodName : "run"});
			this.complete();
			return;
		}
		var imageFile = js_node_Path.join(this.par.inputDir,this.par.cd.tracks[0].diskFile);
		djNode_tools_LOG.log(" - Cutting file " + imageFile + " ",null,{ fileName : "Task_CutTracks.hx", lineNumber : 50, className : "Task_CutTracks", methodName : "run"});
		...
		this.arexec.queue_action = function(tr) {
			...
			cutter.cut(imageFile,js_node_Path.join(_g.par.tempDir,tr.getFilenameRaw()),tr.sectorStart * _g.par.cd.SECTORSIZE,tr.sectorSize * _g.par.cd.SECTORSIZE);
			_g.progress_steps_current = tr.trackNo;
			_g.onStatus("progress",_g);
		};
		this.arexec.queue_complete = function() {
			djNode_tools_LOG.log("Cutting Complete",null,{ fileName : "Task_CutTracks.hx", lineNumber : 70, className : "Task_CutTracks", methodName : "run"});
			if(_g.flag_delete_source) {
				djNode_tools_LOG.log("Deleting image file \"" + imageFile + "\"",null,{ fileName : "Task_CutTracks.hx", lineNumber : 73, className : "Task_CutTracks", methodName : "run"});
				js_node_Fs.unlinkSync(imageFile);
			}
			_g.complete();
		};
		this.arexec.start();
...
Task_$JoinTracks.prototype = $extend(djNode_task_Task.prototype,{
	run: function() {
		...
		this.progress_steps_total = this.par.cd.tracks_total;
		djNode_task_Task.prototype.run.call(this);
		djNode_tools_LOG.log("Joining tracks to an image. Total tracks " + this.par.cd.tracks_total,null,{ fileName : "Task_JoinTracks.hx", lineNumber : 32, className : "Task_JoinTracks", methodName : "run"});
		if(this.par.cd.isMultiImage) {
			djNode_tools_LOG.log("- NO need to JOIN, Track is multitrack",null,{ fileName : "Task_JoinTracks.hx", lineNumber : 35, className : "Task_JoinTracks", methodName : "run"});
			this.complete();
			return;
		}
		if(this.par.cd.tracks_total == 1) {
			djNode_tools_FileTool.moveFile(js_node_Path.join(this.par.tempDir,this.par.cd.tracks[0].filename),this.par.imagePath,function() {
				_g.complete();
			});
			return;
		}
		...
		this.joiner.events.once("close",function(st) {
			if(st) {
				djNode_tools_LOG.log("Join Complete",null,{ fileName : "Task_JoinTracks.hx", lineNumber : 56, className : "Task_JoinTracks", methodName : "run"});
				_g.complete();
			} else {
				djNode_tools_LOG.log("Join ERROR - " + _g.joiner.error_log,null,{ fileName : "Task_JoinTracks.hx", lineNumber : 60, className : "Task_JoinTracks", methodName : "run"});
...
Task_$MoveFiles.prototype = $extend(djNode_task_Task.prototype,{
	run: function() {
		...
		this.arexec.queue_action = function(tr) {
			...
			var destPath = js_node_Path.join(_g.par.outputDir,tr.diskFile);
			djNode_tools_FileTool.moveFile(sourcePath,destPath,function() {
				djNode_tools_LOG.log(" Moved file " + sourcePath + " to " + destPath,null,{ fileName : "Task_MoveFiles.hx", lineNumber : 43, className : "Task_MoveFiles", methodName : "run"});
				_g.arexec.next();
			});
			_g.progress_steps_current = tr.trackNo;
			_g.onStatus("progress",_g);
		...
		this.arexec.queue_complete = function() {
			djNode_tools_LOG.log("Move Complete",null,{ fileName : "Task_MoveFiles.hx", lineNumber : 52, className : "Task_MoveFiles", methodName : "run"});
			_g.complete();
		};
		this.arexec.start();
	}
...
Task_$RestoreTrack.prototype = $extend(djNode_task_Task.prototype,{
	...
	,postRestore: function() {
		if(this.flag_delete_old) try {
			djNode_tools_LOG.log("Deleting " + this.trackFullPath,null,{ fileName : "Task_RestoreTrack.hx", lineNumber : 72, className : "Task_RestoreTrack", methodName : "postRestore"});
			js_node_Fs.unlinkSync(this.trackFullPath);
		} catch( e ) {
			if (e instanceof js__$Boot_HaxeError) e = e.val;
			djNode_tools_LOG.log("Unable to delete.",2,{ fileName : "Task_RestoreTrack.hx", lineNumber : 75, className : "Task_RestoreTrack", methodName : "postRestore"});
		}
		this.track.filename = this.track.getFilenameRaw();
		djNode_tools_LOG.log("Altering new track filename to \"" + this.track.filename + "\"",null,{ fileName : "Task_RestoreTrack.hx", lineNumber : 80, className : "Task_RestoreTrack", methodName : "postRestore"});
		this.trackFullPath = js_node_Path.join(this.shared.tempDir,this.track.filename);
		if(!this.track.isData) {
			var targetSize = this.track.sectorSize * this.shared.cd.SECTORSIZE | 0;
			var trackSize;
			djNode_tools_LOG.log("Fixing size :: ",null,{ fileName : "Task_RestoreTrack.hx", lineNumber : 94, className : "Task_RestoreTrack", methodName : "postRestore"});
			trackSize = Std["int"](js_node_Fs.statSync(this.trackFullPath).size);
			djNode_tools_LOG.log("  pre size = " + trackSize,null,{ fileName : "Task_RestoreTrack.hx", lineNumber : 96, className : "Task_RestoreTrack", methodName : "postRestore"});
			js_node_Fs.truncateSync(this.trackFullPath,targetSize);
			trackSize = Std["int"](js_node_Fs.statSync(this.trackFullPath).size);
			djNode_tools_LOG.log("  post size = " + trackSize,null,{ fileName : "Task_RestoreTrack.hx", lineNumber : 99, className : "Task_RestoreTrack", methodName : "postRestore"});
...
djNode_app_AppSpawner.prototype = {
	...
	,listen_exit: function() {
		...
		this.proc.once("close",function(exit,sig) {
			if(exit != 0 || sig != null) {
				...
				djNode_tools_LOG.log("Child Process Close - [ OK ]",null,{ fileName : "AppSpawner.hx", lineNumber : 132, className : "djNode.app.AppSpawner", methodName : "listen_exit"});
	...
	,checkApp: function() {
		djNode_tools_LOG.log("Checking app..",null,{ fileName : "AppSpawner.hx", lineNumber : 159, className : "djNode.app.AppSpawner", methodName : "checkApp"});
	...
	,onAppCheckResult: function(status,msg) {
		...
		if(status == true) {
			djNode_tools_LOG.log("AppCheck [OK]",null,{ fileName : "AppSpawner.hx", lineNumber : 196, className : "djNode.app.AppSpawner", methodName : "onAppCheckResult"});
			this.events.emit("check",true);
		} else {
			djNode_tools_LOG.log("AppCheck [ERROR]",null,{ fileName : "AppSpawner.hx", lineNumber : 199, className : "djNode.app.AppSpawner", methodName : "onAppCheckResult"});
			this.events.emit("check",false,msg);
		}
	}
	,check_allArch_onpath: function(exeToCheck) {
		var _g = this;
		djNode_tools_LOG.log("Checking on Path for " + exeToCheck,null,{ fileName : "AppSpawner.hx", lineNumber : 210, className : "djNode.app.AppSpawner", methodName : "check_allArch_onpath"});
...
djNode_tools_CDInfo.prototype = {
	...
	,load: function(input) {
		djNode_tools_LOG.log("CDInfo, loading " + input,null,{ fileName : "CDInfo.hx", lineNumber : 117, className : "djNode.tools.CDInfo", methodName : "load"});
		...
		djNode_tools_LOG.log("Guessed cd title = " + this.TITLE,null,{ fileName : "CDInfo.hx", lineNumber : 135, className : "djNode.tools.CDInfo", methodName : "load"});
	...
	,postParse_check: function() {
		...
		djNode_tools_LOG.log("CD Type = " + this.TYPE,null,{ fileName : "CDInfo.hx", lineNumber : 163, className : "djNode.tools.CDInfo", methodName : "postParse_check"});
		djNode_tools_LOG.log("Number of tracks = " + this.tracks.length,null,{ fileName : "CDInfo.hx", lineNumber : 164, className : "djNode.tools.CDInfo", methodName : "postParse_check"});
		...
		if(cc == this.tracks.length && cc > 1) {
			...
			djNode_tools_LOG.log(" Cue Sheet is MULTI FILE ",null,{ fileName : "CDInfo.hx", lineNumber : 219, className : "djNode.tools.CDInfo", methodName : "postParse_check"});
			this.postParse_Multi();
		} else if(cc == 1) {
			this.isMultiImage = false;
			djNode_tools_LOG.log(" Cue Sheet is SINGLE FILE ",null,{ fileName : "CDInfo.hx", lineNumber : 224, className : "djNode.tools.CDInfo", methodName : "postParse_check"});
	...
	,parser_ccd: function(line) {
		...
		if(regGetTrackNo.match(line)) {
			...
			this.tracks.push(tr);
			this.openTrack = tr;
			djNode_tools_LOG.log("discovered Track - " + Std.string(tr),null,{ fileName : "CDInfo.hx", lineNumber : 352, className : "djNode.tools.CDInfo", methodName : "parser_ccd"});
			this.tracks_total++;
			return;
		}
		...
		if(regGetMode.match(line)) {
			...
			case "0":
				...
				djNode_tools_LOG.log("AUDIO - ",null,{ fileName : "CDInfo.hx", lineNumber : 361, className : "djNode.tools.CDInfo", methodName : "parser_ccd"});
				break;
			case "2":
				this.openTrack.type = "MODE2/2352";
				djNode_tools_LOG.log("discovered Track - MODE2/2352",null,{ fileName : "CDInfo.hx", lineNumber : 362, className : "djNode.tools.CDInfo", methodName : "parser_ccd"});
				break;
			}
			return;
		}
	...
	,saveAs_Cue: function(output,comment) {
		...
		if(this.TITLE == null) {
			djNode_tools_LOG.log("Title is null, autoset to 'untitled'",2,{ fileName : "CDInfo.hx", lineNumber : 394, className : "djNode.tools.CDInfo", methodName : "saveAs_Cue"});
			this.TITLE = "untitled";
		}
	...
	,createFromImage: function(filename) {
		throw new js__$Boot_HaxeError("This is broken");
		var image_path = "";
		this.tracks = [];
		this.tracks_total = 0;
		var rtitle = new EReg("([^/\\\\]*)\\.(?:bin|iso|img)$","i");
		if(rtitle.match(filename)) this.TITLE = rtitle.matched(1);
		djNode_tools_LOG.log("createFromImage() - Guessed cd title = " + this.TITLE,null,{ fileName : "CDInfo.hx", lineNumber : 447, className : "djNode.tools.CDInfo", methodName : "createFromImage"});
		image_path = filename;
		var imext = djNode_tools_FileTool.getFileExt(image_path);
		if(HxOverrides.indexOf(["bin","img","iso"],imext,0) < 0) throw new js__$Boot_HaxeError("createFromImage, unsupported Image Extension");
		if(!djNode_tools_FileTool.pathExists(image_path)) throw new js__$Boot_HaxeError("createFromImage(), " + image_path + " - does not exist");
		var size = js_node_Fs.statSync(image_path).size;
		var sectors = size / this.SECTORSIZE;
		if(sectors % this.SECTORSIZE > 0) throw new js__$Boot_HaxeError("Size mismatch error, \"" + image_path + "\" should be of size multiple of " + this.SECTORSIZE);
	...
	,self_save: function(filename) {
		...
		var o = { cdTitle : this.TITLE, sectorSize : this.SECTORSIZE, imageSize : this.total_size, version : djNode_tools_CDInfo.VERSION, tracks : this.tracks};
	...
	,loadSettingsFile: function(filename) {
		...
		djNode_tools_LOG.log("CDInfo restoring data - " + filename,null,{ fileName : "CDInfo.hx", lineNumber : 504, className : "djNode.tools.CDInfo", methodName : "loadSettingsFile"});
		...
		djNode_tools_LOG.log("Title = " + this.TITLE,null,{ fileName : "CDInfo.hx", lineNumber : 551, className : "djNode.tools.CDInfo", methodName : "loadSettingsFile"});
		djNode_tools_LOG.log("Tracks Total = " + Std.string(this.tracks_total),null,{ fileName : "CDInfo.hx", lineNumber : 552, className : "djNode.tools.CDInfo", methodName : "loadSettingsFile"});
		djNode_tools_LOG.log("SectorSize = " + Std.string(this.SECTORSIZE),null,{ fileName : "CDInfo.hx", lineNumber : 553, className : "djNode.tools.CDInfo", methodName : "loadSettingsFile"});
	}
	,getCueName: function() {
		return this.TITLE + ".cue";
	}
...
djNode_tools_CueTrack.prototype = {
	...
	,debugInfo: function() {
		djNode_tools_LOG.log("-Track:" + this.trackNo + " | diskFile:" + this.diskFile + " | diskFileSize:" + this.diskFileSize + " | ",null,{ fileName : "CDInfo.hx", lineNumber : 734, className : "djNode.tools.CueTrack", methodName : "debugInfo"});
		djNode_tools_LOG.log("- indexTot:" + this.indexTotal + " | sector:" + this.sectorSize + " | sectorStart:" + this.sectorStart + " | isData:" + Std.string(this.isData),null,{ fileName : "CDInfo.hx", lineNumber : 735, className : "djNode.tools.CueTrack", methodName : "debugInfo"});
	}
...
djNode_tools_FileTool.moveFile = function(source,dest,onComplete,onProgress) {
	js_node_Fs.rename(source,dest,function(er) {
		if(er != null) djNode_tools_FileTool.copyFile(source,dest,function() {
			try {
				js_node_Fs.unlinkSync(source);
			} catch( e ) {
				if (e instanceof js__$Boot_HaxeError) e = e.val;
				djNode_tools_LOG.log("Could not delete \"" + source + "\" while moving",null,{ fileName : "FileTool.hx", lineNumber : 96, className : "djNode.tools.FileTool", methodName : "moveFile"});
			}
			onComplete();
		},onProgress); else onComplete();
	});
};
...
CDC.flag_overwrite = false;
CDC.flag_res_to_folders = false;
Job_$Restore.DIR_POSTFIX = " (restored)";
